...
function create(item_detail) {
    ...
    if (item) {
        ...
        // making input feild READ ONLY to ensure only one item is sending through from...
...
function getItemDetailsByItem() {
    ...
    if (item) {
        ...
        $.ajax({
            ...
            url: item_info_subcontract_url + item, // item_info_url from lib>js>config.js
            success: function (response) {
                console.log(response);
                if (response.success) {
                    if(response.qc_date === null) {
                        ...
                        response.machine_code = 'Sub-Contract';
